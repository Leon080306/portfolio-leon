...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Nav links — needs ::after */
  .nl { color: #707070; text-decoration: none; font-size: 13.5px; font-weight: 400; position: relative; transition: color .2s; }
  .nl::after { content: ''; position: absolute; bottom: -3px; left: 0; width: 0; height: 1px; background: #7A8C7B; transition: width .3s; }
  .nl:hover { color: #1A1A1A; }
  .nl:hover::after { width: 100%; }

  /* Buttons */
  .bp { background: #222; color: #FAFAF7; border: none; padding: 13px 28px; border-radius: 100px; font-family: 'Sora', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: all .25s; text-decoration: none; }
  .bp:hover { background: #3A3A3A; transform: translateY(-1px); box-shadow: 0 8px 24px rgba(34,34,34,.2); }
  .bs { background: transparent; color: #222; border: 1px solid #D4D4D0; padding: 13px 28px; border-radius: 100px; font-family: 'Sora', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: all .25s; text-decoration: none; }
  ...
  /* Feature cards: 2-col on tablet, 1-col on mobile */
  @media (max-width: 1023px) { .fc-grid { grid-template-columns: 1fr 1fr !important; } }
  @media (max-width: 639px)  { .fc-grid { grid-template-columns: 1fr !important; } }
